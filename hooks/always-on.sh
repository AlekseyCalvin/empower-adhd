...
flag_path="$claude_dir/.empower-adhd-always"
...
skill_path="$script_dir/../skills/empower-adhd/SKILL.md"
[ -f "$skill_path" ] || exit 0
...
printf 'ADHD POWERS ACTIVE (always-on). The ruleset below applies to every response. "stop adhd powers" turns it off for this session; delete %s to turn always-on off for good.\n\n%s\n' \
  "$flag_path" "$body"
